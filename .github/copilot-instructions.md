...
- **Test Validation**: `make test-agda` compares `generated.rs` with `rust/main.rs` using `diff -u`
...
├── agda/
│   └── Main.agda                # AST definition, code generator, and main entry point
...
└── test_generation.sh           # Alternative test script (not used in CI)
```

### Key Files to Modify
- **Rust code**: `rust/main.rs` (the actual implementation)
- **Agda code**: `agda/Main.agda` (AST definition, code generator, and main entry point)
...
2. **Agda test**: Ensure `make test-agda` passes (generated code matches main.rs)
3. **Rust test**: Ensure `make test-rust` passes
4. **If modifying Agda**: Regenerate main.rs with `./agda-build/Main > rust/main.rs` if you want the generated code to become the new implementation
